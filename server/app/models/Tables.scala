...
trait Tables {
  ...
  case class AnswerGradeRow(id: Int, userid: Int, courseid: Int, paaid: Int, percentCorrect: Double, comments: String)
  ...
  implicit def GetResultAnswerGradeRow(implicit e0: GR[Int], e1: GR[Double], e2: GR[String]): GR[AnswerGradeRow] = GR{
    prs => import prs._
    AnswerGradeRow.tupled((<<[Int], <<[Int], <<[Int], <<[Int], <<[Double], <<[String]))
  }
  ...
  class AnswerGrade(_tableTag: Tag) extends profile.api.Table[AnswerGradeRow](_tableTag, "answer_grade") {
    def * = (id, userid, courseid, paaid, percentCorrect, comments) <> (AnswerGradeRow.tupled, AnswerGradeRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(userid), Rep.Some(courseid), Rep.Some(paaid), Rep.Some(percentCorrect), Rep.Some(comments))).shaped.<>({r=>import r._; _1.map(_=> AnswerGradeRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
